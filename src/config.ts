...
// Build browser args with optional env overrides
const computedBrowserArgs = (() => {
  const baseArgs = process.env.BROWSER_ARGS
  ...
  if (process.env.BROWSER_USER_AGENT)
    baseArgs.push(`--user-agent=${process.env.BROWSER_USER_AGENT}`);
  if (process.env.BROWSER_LANG)
    baseArgs.push(`--lang=${process.env.BROWSER_LANG}`);
  if (process.env.BROWSER_WINDOW_SIZE)
    baseArgs.push(`--window-size=${process.env.BROWSER_WINDOW_SIZE}`);
  if (process.env.BROWSER_STEALTH === 'true')
...
const defaultConfig = {
  ...
  archive: {
    ...
    waitTime: process.env.ARCHIVE_WAIT_TIME
      ? parseInt(process.env.ARCHIVE_WAIT_TIME, 10)
      : 10,
    daysToArchive: process.env.ARCHIVE_DAYS_TO_ARCHIVE
      ? parseInt(process.env.ARCHIVE_DAYS_TO_ARCHIVE, 10)
      : 45,
  },
  log: {
  ...
  createOptions1: {
    ...
    browserArgs: computedBrowserArgs,
    puppeteerOptions: {
      puppeteerOptions: {
        userDataDir: process.env.PUPPETEER_USER_DATA_DIR || './userDataDir/',
  ...
  aws_s3: {
    ...
    forcePathStyle:
      process.env.AWS_S3_FORCE_PATH_STYLE === 'true' ? true : null,
  },
};
...
if (!loadedConfigPath) {
  console.log(
    'No external configuration file found. Using default configuration with environment variables.'
  );
}
